...
def recovery_factor(Raw_Data_preprocessed, stats_ROIP):
    well_count = np.arange(len(Raw_Data_preprocessed))
    Raw_Data_preprocessed_ = []

    for well, well_data in zip(well_count, Raw_Data_preprocessed):
        ROIP = stats_ROIP[well]
        well_data['Cum Oil [m3]'] = well_data['Oil [m3/d]'].cumsum()
        data = well_data['Cum Oil [m3]'] / ROIP
        well_data.insert(1, 'RF', data)
        
        Raw_Data_preprocessed_.append(well_data)
    return Raw_Data_preprocessed_
...
#             prop = pad_static.loc[well, 'Effective_Length':'BottomWater_Oil_Saturation']
#             property_ = prop.values
...
#     property_names = list(prop.index)
