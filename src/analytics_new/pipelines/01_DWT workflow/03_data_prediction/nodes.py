...
def split(data: pd.DataFrame) -> pd.DataFrame:
    label_names = ['Steam [m3/d]', 'Emulsion [m3/d]'] 
...
def validate(dummy2, timesteps, parameters: Dict): 
    import glob, os
...
#         well = well.iloc[:min_well_length]     # use minimum well life
        well = well.iloc[:timesteps]   # daily data 
#         well = well.fillna(0) 
...
#     Standardize static data   
    ...
    all_wells_standardized_properties = scaler_static.transform(properties)  
...
#     wells_steam_rate_predicted = model_1.predict(input_data)
    ...
    wells_steam_rate_predicted = wells_steam_rate_predicted.reshape((number_of_wells, timesteps)).T
...
#     wells_emulsion_rate_predicted = model_2.predict(input_data_model_2)    
    ...
    wells_emulsion_rate_predicted = wells_emulsion_rate_predicted.reshape((number_of_wells, timesteps)).T    
    ...
    wells_steam_rate_actual = all_wells_steam_data.reshape((number_of_wells, timesteps)).T
    wells_emulsion_rate_actual = all_wells_emulsion_data.reshape((number_of_wells, timesteps)).T 
    ...
    for well, file in zip(well_count, files): 
        ...
        steam_rate_actual_predicted = pd.DataFrame(np.vstack((np.array(daily_timesteps), steam_rate_actual, steam_rate_predicted)).T, 
                                                   columns = ["Timestep", "steam rate actual", "steam rate predicted"])  
    ...
    for well, file in zip(well_count, files): 
        ...
        emulsion_rate_actual_predicted  = pd.DataFrame(np.vstack((np.array(daily_timesteps), emulsion_rate_actual,
                                                                  emulsion_rate_predicted)).T, 
                                                       columns = ["Timestep", "emulsion rate actual", "emulsion rate predicted"]) 
